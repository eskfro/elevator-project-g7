...
type Inputs struct {
    LocalPhysicalInfo <-chan elev.ElevatorPhysicalInfo
    NetworkPhysicalInfo <-chan elev.ElevatorPhysicalInfo
}

type Outputs struct {
	Role         chan<- elev.ElevatorRole
	PrimaryID    chan<- int
	AliveList    chan<- elev.AliveList
...
func RoleManager(
	...
	for {
		...
		case newPhysicalInfo := <-in.LocalPhysicalInfo:
			physicalInfo = newPhysicalInfo

		case timedOutID := <-timedOutId:
			aliveList[timedOutID].Role = elev.ER_Dead
			out.AliveList <- aliveList
			out.ResetVersion <- timedOutID
			physicalInfo, aliveList = handleAliveListUpdate(physicalInfo, aliveList, timeStart, out, true, false)

		case heartbeat := <-in.NetworkPhysicalInfo:
			select {
			...
			if wasDead {
				out.ResetVersion <- heartbeat.Id
			}
			// log.Println("[RoleManager] New AliveList update to RoleManager")
			aliveList[heartbeat.Id] = heartbeat
			out.AliveList <- aliveList
			physicalInfo, aliveList = handleAliveListUpdate(physicalInfo, aliveList, timeStart, out, false, wasDead)
...
func handleAliveListUpdate(
	...
	case elev.ER_Backup:
		...
		if shouldBecomePrimary(physicalInfo.Id, aliveList, timeStart) {
			...
			// Send update
			out.AliveList <- aliveList
			out.PrimaryID <- physicalInfo.PrimaryId
			out.Role <- physicalInfo.Role
		...
		if wasDead {
			out.AliveList <- aliveList
			out.PrimaryID <- physicalInfo.PrimaryId
			out.Role <- physicalInfo.Role //IDK ABOUT THIS
		}
