...
type Outputs struct {
	OrderTable       chan<- elev.OrderTable
	OrderTablePacket chan<- elev.OrderTablePacket
}
	...
	for {
		...
		case order := <-orderToReassign:
			...
			if physicalInfo.Role == elev.ER_Primary {
				...
				out.OrderTable <- orderTable
				out.OrderTablePacket <- elev.OrderTablePacket{Id: physicalInfo.Id, OrderTable: orderTable}

			}

		// Directly from rolemanager
		case newAliveList := <-in.AliveList:
			...
			newOrderTable := updateOrderTable(orderTable, rcvOrderTable, physicalInfo.Id, allOrderTables, physicalInfo,
				aliveList, out.OrderTablePacket, startOrderTimer, stopOrderTimer)
		...
		case clearOrders := <-in.ClearOrders:
		...
		case packet := <-in.OrderTablePacket:
