...
type Outputs struct {
	LocalOrderTable chan<- elev.LocalOrderTable
	MotorDir        chan<- elevio.MotorDirection
	Movement        chan<- elev.ElevatorMovement
	ClearOrders     chan<- elev.ClearOrders
}

func Movement(initElev elev.Elevator, in Inputs, out Outputs) {
	...
	for {
		...
		case newPhysicalInfo := <-in.PhysicalInfo:
			...
			if isChanged || physicalInfo.Movement == elev.EM_Idle {
				physicalInfo = fsm_OnTableUpdate(physicalInfo, doorTimer, out)
			}

		case <-doorTimer.C:
			log.Println("[Movement] Doortimer Event")
			physicalInfo = fsm_OnDoorTimeout(physicalInfo, doorTimer, out)
			prevLOT = physicalInfo.LocalOrderTable

		case newFloor := <-in.FloorArrival:
			fmt.Printf("[Movement]: Arrived at Floor = %d\n", newFloor)
			physicalInfo.Floor = newFloor
			physicalInfo = fsm_OnFloorArrival(physicalInfo, doorTimer, out)
			prevLOT = physicalInfo.LocalOrderTable
...
func fsm_OnTableUpdate(
	...
	case elev.EM_DoorOpen:
		...
		if requests.ShouldStop(PhysicalInfo.LocalOrderTable, PhysicalInfo.Floor, PhysicalInfo.MotorDir) {
			...
			if anyOrderToClear(buttonsToClear) {
				sendClearOrder(PhysicalInfo, buttonsToClear, out.ClearOrders)
			}
			// Guard Added 11.03
			if PhysicalInfo.LocalOrderTable != prevLOT {
				out.LocalOrderTable <- PhysicalInfo.LocalOrderTable
			}
			if PhysicalInfo.Movement != prevMovement {
				out.Movement <- PhysicalInfo.Movement
			}
		}
	...
	case elev.EM_Idle:
		...
		if PhysicalInfo.MotorDir != prevMotorDir {
			out.MotorDir <- PhysicalInfo.MotorDir
		}
		if PhysicalInfo.Movement != prevMovement {
			out.Movement <- PhysicalInfo.Movement
		}
